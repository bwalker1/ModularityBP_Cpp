...
def create_multiplex_graph(n_nodes=100, n_layers=5, mu=.99, p=.1, ncoms=10, k_max=150,k_min=3):
    ...
    # with use the degree corrected SBM to mirror paper
    multinet = gm.multilayer_DCSBM_network(partition, mu=mu, k_min=k_min, k_max=k_max, t_k=-4)
    #     return multinet
...
def run_ZMBP_on_graph(graph, q, beta,niters=100):
    ...
    sbmbpfile = os.path.join(clusterdir,'experiments/compare_with_ZM/modbp/mod')
    outdir = os.path.join(clusterdir,'experiments/compare_with_ZM/zm_outdir')
